...
const SurveyAndMapping = () => {
  ...
  return (
    <div className='flex flex-col divide-y space-y-12 relative'>
      <img src='/accent-2.svg' alt='' className='select-none absolute -top-10' />
      <div className=' flex flex-col space-y-8 md:grid md:grid-cols-2  content-center px-[20px] md:px-[120px] mt-16 relative'>
          ...
          <div className='grid grid-rows-2  bg-survey bg-cover bg-center rounded-[1rem] w-[90%]  h-[80%]'>
            <div className='grid grid-cols-2 row-start-2'>
              <div className='outline outline-[16px] md:outline-[28px] bg-white w-[100%]  outline-white rounded-tr-lg'>
                <Image src='/surveyImage2.png' className='' alt='' width='300' height='300' />
      ...
      {/* page2 */}
      <div className='px-[20px] md:px-[120px] flex flex-col space-y-16 relative'>
        <img src='/accent.svg' alt='' className='absolute right-0 top-10' />
      ...
      <div className='px-[20px] md:px-[120px] '>
        <div className='pt-8'>
          <div className='lg:hidden border border-black rounded-lg text-center'>
            <Dropdown title='Select Section' texts={["Field Survey", "Aerial Photo", "UAV Lidar", "GIS", "Thermovision Technology", "Aerial Video", ""]} />
          </div>
        </div>
        ...
        >
            ...
            <div id='aerial' className=' flex flex-col space-y-8 '>
              ...
              <div className='row-span-2 flex flex-col lg:grid lg:grid-cols-2 gap-2 lg:gap-12 '>
                {aerials.map(({ aerial, name, year }) => {
                  return (
                    <div key={name} className=' flex flex-col  p-[20px] lg:p-[40px] space-x-4  space-y-[24px] rounded-[1rem] bg-white shadow-2xl '>
          ...
          <aside className={`hidden lg:flex col-span-1 p-[10%]`}>
            <ul className='flex flex-col space-y-12'>
              <Link href={`#survey`} name='survey' onClick={linkfunc} className={lin == "survey" ? "text-blue" : ""}>
                Field Survey
              </Link>
              <Link href={`#aerial`} name='aerial' onClick={linkfunc} className={lin == "aerial" ? "text-blue" : ""}>
                Aerial Photo
              </Link>
              <Link href={`#lidar`} name='lidar' onClick={linkfunc} className={lin == "lidar" ? "text-blue" : ""}>
                UAV Lidar
              </Link>
              <Link href={`#gis`} name='gis' onClick={linkfunc} className={lin == "gis" ? "text-blue" : ""}>
                GIS
              </Link>
              <Link href={`#thermo`} name='thermo' onClick={linkfunc} className={lin == "thermo" ? "text-blue" : ""}>
                Thermovision Technology
              </Link>
              <Link href={`#aev`} name='aev' onClick={linkfunc} className={lin == "aev" ? "text-blue" : ""}>
                Aerial Video
              </Link>
            </ul>
          </aside>
