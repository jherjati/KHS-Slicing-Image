...
const SurveyAndMapping = ({ pageData }) => {
  ...
  const { title, description, subserviceTitle, subserviceDescription, tools } = pageData;
    ...
    <div className='flex flex-col pt-[70px] relative'>
      <div className='space-y-10 flex flex-col md:space-y-0 md:grid md:grid-cols-2 content-center px-[20px] md:px-[120px] relative border-b md:py-[90px] py-10 m-0 '>
        <div className='flex justify-center flex-col space-y-5 md:space-y-8 col-span-1 mx-3'>
          ...
          <h1 className='text-blacky text-sedangmobile mt-5 md:mt-0 md:text-sedang2 font-quicksand '>{title}</h1>
          <div className='text-grey text-kecilmobile md:text-kecil font-inter text-justify' dangerouslySetInnerHTML={{ __html: description }}></div>
        ...
        <div className='flex justify-end items-center'>
          <div className='grid grid-rows-2  bg-survey bg-cover bg-center rounded-[1rem] w-full md:w-[90%] h-[80%]'>
            <div className='grid grid-cols-2 row-start-2'>
              <div className='outline  md:outline-[28px] bg-white w-[100%]  outline-white rounded-2xl'>
                <Image src='/surveyImage2.png' className='h-full object-cover ' alt='Image' width='300' height='300' cla />
        ...
        <div className='flex justify-center md:justify-between relative'>
          <div className='row-span-2 grid grid-cols-2 gap-x-12 gap-y-4 md:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-8 w-full'>
            {tools.map(({ title, mediaItemUrl }) => {
              return (
                <div key={title} className='text-center flex flex-col items-center justify-center space-y-4 rounded-[1rem] bg-white shadow-lg w-full p-2 md:p-4'>
                  <div className='relative h-20 w-full'>
                    <Image fill src={mediaItemUrl} alt='Image' className='object-contain object-center' />
                  </div>
                  <p className='pb-2 text-blacky text-[12px] md:text-[14px] leading-[15px] md:leading-[17px] font-[600] font-inter'>{title}</p>
                </div>
              );
            })}
          </div>
      ...
      <div className='px-[20px] md:px-[120px]'>
        <div className='lg:hidden border rounded-lg my-10'>
          <SelectButton onChange={onClickDropDown} selected={lin} options={subserviceTitle.map((item) => ({ id: item, value: item }))} />
        </div>
        <div
        ...
        >
          <div className='col-span-3 flex flex-col space-y-5 md:space-y-8 md:pt-16 lg:pr-[3%]'>
            {subserviceTitle.map((ssTitle, key) => (
              <div key={key} id={ssTitle} name={ssTitle} className=' flex flex-col space-y-5 md:space-y-8' ons>
                <h1 className='text-blacky text-sedangmobile md:text-sedang2 font-quicksand '>{ssTitle}</h1>
                <div
                      ...
                      return (
                        <div key={name} className=' flex flex-col  p-[20px] lg:p-[40px] space-x-4 space-y-[24px] rounded-[1rem] bg-white shadow-lg '>
                          <div className='flex items-center justify-center'>
                            <Image layout='responsive' src={aerial} alt={name} width='356' height='192' className='rounded-md' />
                          </div>
                ...
                {key === 3 && <Image className='rounded-lg' src='/gisimage.png' alt='UAV Image' width='1100' height='400' layout='responsive' />}
                {key === 4 && (
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-5'>
                    <Image className='rounded-lg' src='/thermalinspection1.JPG' alt='UAV Image' width='1100' height='400' layout='responsive' />
                    <Image className='rounded-lg' src='/thermalinspection2.JPG' alt='UAV Image' width='1100' height='400' layout='responsive' />
                  </div>
                )}
                {key === 5 && (
          ...
          <aside className={`hidden lg:flex col-span-1 p-[10%] pt-16 w-full`}>
            <ul className='flex flex-col space-y-5 font-quicksand w-full'>
              {subserviceTitle.map((ssTitle) => (
                <a key={ssTitle} href={"#" + ssTitle} onClick={() => setlin(ssTitle)} className={lin === ssTitle ? "text-blue font-bold flex items-center gap-2 " : "group flex hover:text-blue-dark hover:font-bold items-center gap-2"}>
                  <div className={clsx("h-1 rounded w-4 bg-blue-dark group-hover:block", { block: lin === ssTitle, hidden: lin !== ssTitle })} />
                  {ssTitle}
                </a>
              ))}
            </ul>
