...
const ArchitectureConstructionManagement = ({ pageData }) => {
  ...
  const { title, description, subserviceTitle, subserviceDescription, subserviceProjects } = pageData;
    ...
    <div className='flex flex-col pt-[70px] relative'>
      <div className='space-y-10 flex flex-col md:space-y-0 md:grid md:grid-cols-2 content-center px-[20px] md:px-[120px] relative border-b md:py-[90px] py-10 m-0 '>
        <div className='flex justify-center flex-col space-y-5 md:space-y-8 col-span-1 mx-3'>
          ...
          <h1 className='text-blacky text-sedangmobile md:text-sedang2 font-quicksand  '>{title}</h1>
          ...
          <div className='grid grid-rows-2 w-full bg-architecture bg-cover bg-center rounded-[1rem] md:w-[90%]  h-[80%]'>
            <div className='grid grid-cols-2 row-start-2'>
              <div className='outline outline-[16px] md:outline-[28px] bg-white w-[100%]  outline-white rounded-2xl'>
                <Image src='/architectureimage.png' className='h-full object-cover ' alt='Image' width='300' height='300' />
              ...
              <div key={key} id={ssTitle} className=' flex flex-col space-y-8 '>
                ...
                <div className='row-span-2 flex flex-col space-y-3 md:space-y-0 md:grid md:grid-cols-3 gap-2 md:gap-12 '>
                  {subserviceProjects
                    ...
                    .map(({ mediaItemId, mediaItemUrl, altText, caption, description }) => {
                      return (
                        <div key={mediaItemId} className=' flex flex-col p-5 space-x-4  space-y-[24px] rounded-[1rem] bg-white shadow-xl cursor-pointer' onClick={() => router.push(`/service/architecture-construction/${mediaItemId}`)}>
                          <div className='relative w-full aspect-video'>
                            <Image fill src={mediaItemUrl} alt={altText} className='object-cover object-center' />
                          </div>
          ...
          <aside className='hidden md:flex col-span-1 pt-16 p-[10%] '>
            <ul className='space-y-5 '>
              {subserviceTitle.map((ssTitle) => (
                <a key={ssTitle} href={"#" + ssTitle} className={lin === ssTitle ? "text-blue font-bold flex items-center gap-2 " : "group flex hover:text-blue-dark hover:font-bold items-center gap-2"} onClick={() => setlin(ssTitle)}>
                  <div
                    className={clsx("h-1 rounded w-4 bg-blue-dark group-hover:block", {
                      block: lin === ssTitle,
                      hidden: lin !== ssTitle,
                    })}
                  />
